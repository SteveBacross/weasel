...
def generate_dependency_graph_dash(
    ...
    vuln_set = set(name.lower() for name in vulnerabilities_map.keys())
    ...
    for entry in tree_data:
        ...
        vuln_list = vulnerabilities_map.get(key, [])
        ...
        for dep in entry.get("dependencies", []):
            child_raw = dep.get("package_name") or dep.get("key")
            if not child_raw:
                continue
            edges.append({ "data": { "source": raw_name, "target": child_raw } })
...
<head>
    ...
    <style>
        ...
        #cytoscape {{ width:100vw; height:100vh; display:block; }}
        #sidebar {{
...
<body>
    ...
    <script>
        ...
        let filterOn = false;
        document.getElementById('filter-vuln').addEventListener('click', () => {{
            if (!filterOn) {{
